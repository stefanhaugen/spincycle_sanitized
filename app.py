...
def _resolve_dropbox_dest() -> str:
    ...
    try:
        if hasattr(st, "secrets"):
            ...
            if v: return _normalize_dropbox_path(str(v))
    except Exception:
        pass
...
def render_dropbox_push_ui(key_prefix: str, meta: dict, fallback_stem: str,
    ...
    if tok is None:
        new_tok = st.text_input(
            "Dropbox access token", type="password",
    ...
    suffix = st.text_input(
        "Filename suffix (optional)", value="",
    ...
    if not can_preview:
        st.button("👁 Preview push", key=f"{key_prefix}dbx_preview_btn",
                  disabled=True,
                  help=("Build the Excel first." if xlsx_bytes is None
    ...
    if prev:
        with st.container(border=True):
            ...
            st.markdown(f"**Filename:** `{prev['final_name']}` &nbsp; {badge}",
            ...
            cA, cB = st.columns(2)
            with cA:
                if st.button("✅ Confirm & upload", key=f"{key_prefix}dbx_confirm",
                             type="primary"):
                    with st.spinner("Uploading…"):
                        try:
                            full = _dbx_upload(tok, prev["dest"],
                                                prev["final_name"], prev["data"])
...
INSTRUMENT_NAMES = [
    "", "R2P2", "Yoshi", "Crush", "WallE", "Spider",
    "BB8", "Luigi", "Mario", "Peach",
    "Black Dragon", "Red Dragon", "Eve", "Toucan", "Groot", "D-0", "Rocket"
    ]
...
def classify_sample(name: str) -> str:
    ...
    if "std_" in n or "std-" in n or n.startswith("std"):  return "STD"
    if "cvs_" in n or "cvs-" in n or n.startswith("cvs"):  return "CVS"
...
def flag_lloq_uloq(value, lloq, uloq):
    ...
    try:
        v = float(value)
        if pd.notna(lloq) and v < float(lloq): return "< LLOQ"
        if pd.notna(uloq) and v > float(uloq): return "> ULOQ"
        return "In Range"
    except (TypeError, ValueError):
        return ""


def decide_cell_style(rtype: str, sample_name: str, known,
                      ref_val, analyte: str,
                      std_lo: float, std_hi: float,
                      cvs_tolerances: dict,
                      analyte_assignment: dict,
                      lloq_map: dict, uloq_map: dict) -> str:
    ...
    if rtype == "STD":
        if known is None or known <= 0:           return "neutral"
        pct = rv / known * 100.0
    ...
    if rtype == "Sample":
        ...
        if lloq is None and uloq is None:         return "neutral"
        if lloq is not None and rv < float(lloq): return "lloq"
        if uloq is not None and rv > float(uloq): return "fail"
        return "pass"
    return "neutral"
        ...
        if "Submission Sample Table" in wb.sheetnames:
            ...
            header_idx = next(
                (i for i, r in enumerate(rows)
                 if any(str(c).lower() in ("sample number","sample name on vial")
...
def cached_parse_chemstation(file_bytes: bytes, _ver=_CACHE_VER) -> tuple:
    ...
    raw_df = data_df.copy()  # FIX: was data_df.copy() — ensure mutations below don't corrupt raw sheet
    ...
    keep = (["Sample","Location","Dilution Factor","isStandard","STD_Known_Conc","CVS_Known_Conc","Run"]
...
def _find_masshunter_sheet(file_bytes: bytes) -> str:
    ...
    for sn in candidates:
        try:
            row0 = pd.read_excel(BytesIO(file_bytes), sheet_name=sn,
...
def cached_parse_masshunter(file_bytes: bytes, _ver=_CACHE_VER) -> tuple:
    ...
    data["Dilution Factor"] = pd.to_numeric(data.get("Dilution Factor"),
    ...
    base = [c for c in ["Sample Name", "Location", "Dilution Factor",
                        "Type", "Level",
                        "isStandard", "STD_Known_Conc", "CVS_Known_Conc",
                        "Acq DateTime"]
...
@st.cache_data(show_spinner=False)
def cached_cvs_workup(tidy_json: str, unit: str, cvs_tol_json: str,
...
@st.cache_data(show_spinner=False)
def cached_merge_submission(tidy_json: str,
                             sub_meta_json: str,
                             sub_samples_json: str, _ver=_CACHE_VER) -> str:
    df       = safe_read_json(tidy_json)
    ...
    if sub_samples_json:
        try:
            ...
            sub_df = pd.DataFrame()
        if not sub_df.empty:
            vial_col = next((c for c in sub_df.columns
                             if "sample name" in str(c).lower() or
                             "vial" in str(c).lower()), None)
            if vial_col:
                sub_df["_mk"] = sub_df[vial_col].astype(str).str.lower().str.strip()
                df["_mk"]     = df["Sample Name"].astype(str).str.lower().str.strip()
                extra = [c for c in sub_df.columns
                         if c not in (vial_col,"_mk","Sample number")]
                for col in extra:
                    df[f"sub_{col}"] = None
...
def build_workup(tidy_df: pd.DataFrame,
                 lloq_map: dict, uloq_map: dict,
                 unit: str,
                 std_lo: float = 90, std_hi: float = 110,
                 cvs_tolerances: dict | None = None,
                 analyte_assignment: dict | None = None) -> tuple:
    """
    ...
    if include_type:  cols.append("Type")
    if include_level: cols.append("Level")
    ...
    for analyte, _raw_col, _corr_col in pairs:
        lloq = lloq_map.get(analyte)
        uloq = uloq_map.get(analyte)
        lbl  = (f"LLOQ={lloq if lloq is not None else 'n/a'} | "
...
def build_summary(workup_df: pd.DataFrame, row_types: list, *,
                  use_dilution: bool,
                  include_blanks: bool,
                  include_std: bool,
                  include_cvs: bool,
    ...
    keep_mask = []
    for rt in row_types:
        if rt == "ref":              keep_mask.append(True)
        elif rt == "sample":         keep_mask.append(True)
        elif rt == "blank":          keep_mask.append(include_blanks)
        elif rt == "std":            keep_mask.append(include_std)
        elif rt == "cvs":            keep_mask.append(include_cvs)
        elif rt == "sep":            keep_mask.append(False)
        else:                        keep_mask.append(False)
...
def render_std_qc_bounds(key_prefix: str) -> tuple:
    ...
    c1, c2 = st.columns(2)
    with c1:
        std_lo = st.number_input("STD Recovery Lower (%)", value=90, step=1,
                                 key=f"{key_prefix}std_lo")
    with c2:
        std_hi = st.number_input("STD Recovery Upper (%)", value=110, step=1,
                                 key=f"{key_prefix}std_hi")
...
def render_inclusion_checkboxes(key_prefix: str) -> tuple:
    ...
    with c1:
        use_dil    = st.checkbox("Include dilution workup in Summary",
                                 value=True,  key=f"{key_prefix}use_dil")
    with c2:
        inc_blanks = st.checkbox("Include blank injections in Summary",
                                 value=False, key=f"{key_prefix}inc_blanks")
    with c3:
        inc_std    = st.checkbox("Include STD assessment in Summary",
                                 value=False, key=f"{key_prefix}inc_std")
    with c4:
        inc_cvs    = st.checkbox("Include CVS assessment in Summary",
                                 value=False, key=f"{key_prefix}inc_cvs")
    return use_dil, inc_blanks, inc_std, inc_cvs


def render_cvs_analyte_assignment_editor(cvs_names: list, analytes: list,
                                          key_prefix: str,
                                          *,
    ...
    for n in cvs_names:
        ...
        if b not in seen:
            seen.add(b); base_names.append(b)
    ...
    use_grouping = (
        len(base_to_conc) == len(base_names)   # every base has a conc
        and 0 < len(unique_concs) < len(base_names)  # grouping is reductive
    )
    ...
    def _build_default() -> pd.DataFrame:
        ...
        for a in analytes:
            ...
            for label in column_labels:
                ...
                row[label] = any(a_lc in base.lower()
                                  for base in grouping_map[label])
            rows.append(row)
    ...
    for label in column_labels:
        ...
        if use_grouping and len(underlying) > 1:
            help_txt = ("Applies to " + str(len(underlying))
                        + " injection(s): " + ", ".join(underlying))
        else:
            help_txt = None
...
def render_refresh_button(key_prefix: str):
    ...
    if st.button("🔄 Refresh QC", key=f"{key_prefix}refresh_qc",
                 help="Re-render the heatmap with current tolerances and assignments."):
        st.rerun()


def memoized_build_workup(cache_key: str, session_key: str,
                          tidy_df, lloq_map, uloq_map, unit,
                          std_lo, std_hi, cvs_tolerances,
                          analyte_assignment=None) -> tuple:
...
def render_workup_table(workup_df: pd.DataFrame, row_types: list,
                        unit: str,
    ...
    for col, analyte, is_corrected in value_cols:
        if is_corrected: corr_vals_by_analyte[analyte] = col
        else:            raw_vals_by_analyte[analyte]  = col
    ...
    SEP_STYLE        = "background-color: #e9ecef; border-top: 1px solid #adb5bd; border-bottom: 1px solid #adb5bd"
    ...
    def _decide_value_style(row_idx: int, col: str, analyte: str) -> str:
        ...
        decision = decide_cell_style(
            ...
            cvs_tolerances=cvs_tolerances,
            analyte_assignment=analyte_assignment,
            lloq_map=lloq_map, uloq_map=uloq_map,
        )
    ...
    def style_row(idx: int) -> list:
        ...
        if rtype == "ref":   return [REF_STYLE] * n
        if rtype == "sep":   return [SEP_STYLE] * n
        if rtype == "blank": return [BLANK_INJ_STYLE] * n
        styles = [""] * n
    ...
    def _styler(_df):
        return pd.DataFrame(
            ...
            index=workup_df.index, columns=workup_df.columns,
        )

    def _fmt_value(v):
        if isinstance(v, bool): return v
        if isinstance(v, (int, float)) and pd.notna(v): return f"{v:.3f}"
        return v if v is not None else ""

    fmt_targets = [c for c, _, _ in value_cols]
    if "Known Conc" in workup_df.columns: fmt_targets.append("Known Conc")
    styled = (workup_df.style
              .apply(_styler, axis=None)
              .format(_fmt_value, subset=fmt_targets, na_rep=""))
...
def render_qc_heatmap(std_df: pd.DataFrame, cvs_df: pd.DataFrame,
                      std_lo: float, std_hi: float,
                      cvs_tolerances: dict):
    """Render a single stacked QC heatmap: STD rows on top, CVS rows below."""
    has_std = (not std_df.empty and "% Recovery" in std_df.columns
               and "Analyte" in std_df.columns)
    has_cvs = (not cvs_df.empty and "% Recovery" in cvs_df.columns
               and "Analyte" in cvs_df.columns)
    ...
    if has_std:
        std_pivot = std_df.pivot_table(
            index="Sample Name", columns="Analyte",
            values="% Recovery", aggfunc="first"
        )
    ...
    all_analytes = sorted(
        set(std_pivot.columns.tolist()) | set(cvs_pivot.columns.tolist())
    )
    ...
    if not cvs_pivot.empty:
        ...
        if not std_pivot.empty:
            ...
            divider = pd.DataFrame(
                [[np.nan] * len(all_analytes)],
                columns=all_analytes,
                index=["─── ─── ───"]
            )
    ...
    styled = combined.style.apply(lambda _: _apply_styles(combined),
                                   axis=None).format("{:.1f}%", na_rep="—")
...
def build_excel(tidy_df, raw_df,
                workup_df, workup_row_types,
                summary_df, summary_row_types,
                meta: dict, submission_wb_bytes=None,
                qc_context: dict | None = None) -> bytes:
    """
    ...
    def _color_value_cells(ws, df, row_types, row_meta,
                            ctx_unit, ctx_std_lo, ctx_std_hi,
                            ctx_cvs_tol, ctx_assignment,
                            ctx_lloq, ctx_uloq):
        ...
        rtype_to_label = {"std": "STD", "cvs": "CVS",
                          "sample": "Sample", "blank": "Blank", "ref": "ref", "sep": "sep"}
        ...
        for i, rtype in enumerate(row_types):
            ...
            for analyte, (raw_idx, corr_idx) in by_analyte.items():
                ...
                decision = decide_cell_style(
                    ...
                    sample_name=name, known=known,
                    ref_val=ref_val, analyte=analyte,
                    std_lo=ctx_std_lo, std_hi=ctx_std_hi,
                    cvs_tolerances=ctx_cvs_tol,
                    analyte_assignment=ctx_assignment,
                    lloq_map=ctx_lloq, uloq_map=ctx_uloq,
                )
                if decision == "neutral":
                    continue
                fill, font = {
    ...
    if submission_wb_bytes:
        ...
        for sh in ["Summary","Workup","Tidy Data","STD Workup","CVS Workup",
                   "Raw Data","Method Information"]:
    ...
    if qc_context:
        _color_value_cells(
            ws_wk, workup_df, workup_row_types,
            qc_context.get("workup_row_meta"),
            qc_context.get("unit", ""),
            qc_context.get("std_lo", 90), qc_context.get("std_hi", 110),
            qc_context.get("cvs_tolerances"),
            qc_context.get("analyte_assignment"),
            qc_context.get("lloq_map"), qc_context.get("uloq_map"),
        )
        _color_value_cells(
            ws_sum, summary_df, summary_row_types,
    ...
    # Sheet order
    order  = [s for s in ["Submission Information","Submission Sample Table"]
              if s in wb_out.sheetnames]
    order += [s for s in ["Workup","Summary","Tidy Data","Raw Data","Method Information"]
              if s in wb_out.sheetnames]
...
def build_batch_excel(per_file: list, combined_tidy: pd.DataFrame,
                      per_file_meta: dict,
                      submission_wb_bytes=None,
                      qc_context_by_file: dict | None = None) -> bytes:
    """
        ...
        for i, rtype in enumerate(row_types):
            ...
            for analyte, (raw_idx, corr_idx) in by_analyte.items():
                ...
                decision = decide_cell_style(
                    ...
                    sample_name=name, known=known,
                    ref_val=ref_val, analyte=analyte,
                    std_lo=ctx.get("std_lo", 90), std_hi=ctx.get("std_hi", 110),
                    cvs_tolerances=ctx.get("cvs_tolerances"),
                    analyte_assignment=ctx.get("analyte_assignment"),
                    lloq_map=ctx.get("lloq_map"), uloq_map=ctx.get("uloq_map"),
                )
                if decision == "neutral":
                    continue
                fill, font = {
    ...
    for entry in per_file:
        ...
        if ctx:
            _color_value_cells_local(
                ws_wk, entry["workup_df"], entry.get("workup_types"),
                ctx.get("workup_row_meta"), ctx,
            )
            _color_value_cells_local(
                ws_sm, entry["summary_df"], entry.get("summary_types"),
                ctx.get("summary_row_meta"), ctx,
            )
...
def export_sqlite(runs_meta, tidy_df: pd.DataFrame,
                  path: str = "/tmp/analytical_data.db") -> bytes:
...
def run_processing(file_bytes, inst, meta, cvs_tolerances,
                   sub_meta, sub_samples_json,
                   analyte_assignment: dict | None = None,
                   std_lo: float = 90, std_hi: float = 110):
    """
...
def recompute_qc(tidy_json: str, unit: str,
                 cvs_tolerances: dict, analyte_assignment: dict | None,
                 std_lo: float, std_hi: float) -> tuple:
...
if sub_file is not None:
    ...
    if st.session_state.get("_sub_file_id") != _sub_file_id:
        ...
        st.session_state["submission_samples"]    = sub_samples_json     # legacy contract for run_processing
    ...
    with c1:
        st.success(f"Loaded — Tracker **{_display_meta.get('Tracking Number','?')}** | "
                   f"{len(_display_samples_df)} samples")
        st.dataframe(pd.DataFrame(_display_meta.items(), columns=["Field","Value"]),
                     use_container_width=True, hide_index=True, height=340)
    with c2:
        st.caption("Sample Manifest")
        st.dataframe(_display_samples_df,
...
with _upload_c2:
    _early_upload = st.file_uploader(
        ...
        type=["xlsx", "xls"], key="single_upload",
...
if _early_upload is not None:
    ...
    _eu_id = getattr(_early_upload, "file_id",
                     f"{_early_upload.name}::{_early_upload.size}")
    ...
    for _k in ("_single_extracted_meta", "_single_file_bytes",
               "_single_file_stem", "_single_tidy_preview", "_single_parse_key"):
...
with r2c2:
    ...
    analytical_method = st.text_input("Analytical Method",
    ...
    processing_method = st.text_input("Processing Method", value="",
...
with r2c3:
    ...
    run_date        = st.date_input("Run Date", key="meta_run_date",
                                      help="Auto-populated from the raw file when a date "
                                           "is found there; otherwise today.")
with r2c4:
    instrument_name = st.text_input(
        "Instrument Name",
        value="",
        placeholder="e.g. R2P2 or R2P2; Mario",
        help=("Free-text entry. Use semicolons to separate multiple instruments.\n\n"
              "Suggested names: " + ", ".join(n for n in INSTRUMENT_NAMES if n)),
    )
    
    output_dir = st.text_input("Output folder (optional)", value="", key="output_dir",
...
META = {
    "Tracking Number": tracking, "Analyst": analyst, 
    "Task Number": task_number,  "Task Name": task_name, "Task PI": task_pi,
    "Analysis": analysis, "Batch": batch,
    "Analytical Method": analytical_method,
    "Processing Method": processing_method,
    "Unit": unit, "Column Serial": col_serial, "Instrument Name": instrument_name,
    "Run Date": str(run_date), "Export Type": instrument,
}
st.divider()
for _src, _dst in [
...
def render_lloq_uloq_ui(tidy_df: pd.DataFrame, unit: str) -> tuple:
    ...
    if not std_levels or not analytes:
        ...
        return {a: None for a in analytes}, {a: None for a in analytes}
    ...
    for lvl in std_levels:
        ...
        with row_cols[0]:
            ...
            bc1.button("All LLOQ", key=f"all_lloq_{lvl}",
                       on_click=_check_row, args=(lvl, analytes, "lloq"))
            bc2.button("All ULOQ", key = f"all_uloq_{lvl}",
                       on_click = _check_row, args=(lvl, analytes, "uloq"))
        for i, a in enumerate(analytes):
            with row_cols[i+1]:
    ...
    for i, a in enumerate(analytes):
        with ov_cols[i+1]:
            ov = st.number_input("", min_value=0.0, value=0.0, step=0.001,
                                 format="%.4f", key=f"lloq_ov_{a}",
...
if mode == "Single File":
    ...
    _cvs_names = (
        ...
        if "isStandard" in tidy_preview.columns else []
    )
    ...
    if _cvs_names and _analyte_list:
        ...
        with st.form("single_cvs_assign_form", clear_on_submit=False):
            render_cvs_analyte_assignment_editor(
                _cvs_names, _analyte_list, key_prefix="single_",
                name_to_conc=_cvs_name_to_conc, unit=unit,
            )
            _refresh_clicked = st.form_submit_button(
                "🔄 Refresh QC", type="primary",
    ...
    if st.session_state.get("_single_run_hash") != _run_hash:
        with st.spinner("Running pipeline…"):
            try:
                res = run_processing(
                    file_bytes, instrument, META, cvs_tolerances,
    ...
    std_df, cvs_df = recompute_qc(
        _tidy_json, unit, cvs_tolerances, cvs_analyte_assignment, std_lo, std_hi,
    )
    ...
    if "Pass" in cvs_df.columns and "CVS Level" in cvs_df.columns and len(cvs_df):
        ...
        if cvs_lvl_vals:
            ...
            for i, lvl in enumerate(cvs_lvl_vals):
                sub = cvs_df[(cvs_df["CVS Level"] == lvl)
                              & (cvs_df["Pass"].isin(["✅", "❌"]))]
                lr  = (sub["Pass"] == "✅").mean() * 100 if len(sub) else float("nan")
                tol = cvs_tolerances.get(float(lvl), 10)
                with lc[i % len(lc)]:
                    st.metric(f"CVS {lvl} {unit}  ±{tol}%",
                               f"{lr:.1f}%" if pd.notna(lr) else "—")
    st.divider()
    ...
    use_dil, inc_blanks, inc_std, inc_cvs = render_inclusion_checkboxes(
        key_prefix="single_")
    ...
    with mc2:
        if "Pass" in cvs_df.columns and len(cvs_df):
            ...
            st.metric("CVS Overall Pass Rate", "—")
    with mc3:
        n_blanks = int((tidy_df["isStandard"] == "Blank").sum()) if "isStandard" in tidy_df.columns else 0
        st.metric("Blank Injections", f"{n_blanks}")
    with mc4:
        n_samples = int((tidy_df["isStandard"] == "Sample").sum()) if "isStandard" in tidy_df.columns else 0
    ...
    with t1:
        ...
        render_workup_table(
            workup_df, workup_row_types, unit,
    ...
    with t2:
        kept_bits = []
        if use_dil:    kept_bits.append("dilution-corrected columns")
        if inc_blanks: kept_bits.append("blank injections")
        if inc_std:    kept_bits.append("STD rows")
        if inc_cvs:    kept_bits.append("CVS rows")
        ...
        for rt in workup_row_types:
            if   rt == "ref":      _kept_mask.append(True)
            elif rt == "sample":   _kept_mask.append(True)
            elif rt == "blank":    _kept_mask.append(inc_blanks)
            elif rt == "std":      _kept_mask.append(inc_std)
            elif rt == "cvs":      _kept_mask.append(inc_cvs)
            else:                  _kept_mask.append(False)
        summary_row_meta = [m for m, k in zip(workup_row_meta, _kept_mask) if k]
        render_workup_table(
            summary_df, summary_row_types, unit,
    ...
    color_summary = st.checkbox(
        ...
        value = False, key="color_summary_xlsx",
    ...
    with ec1:
        if st.button("📥 Build Excel", key="xl_build"):
            with st.spinner("Building workbook…"):
                ...
                st.session_state["xlsx_bytes"]    = build_excel(
                    tidy_df, raw_df,
                    workup_df, workup_row_types,
                    summary_df, summary_row_types if color_summary else [],
                    _effective_meta,
                    submission_wb_bytes=st.session_state.get("submission_wb"),
                    qc_context={**_qc_ctx, "summary_row_meta": _qc_ctx["summary_row_meta"] if color_summary else None}
                )
        ...
        if st.session_state["xlsx_bytes"] is not None:
            label = ("⬇️ Download Excel (+ submission form)"
                     if st.session_state.get("submission_wb")
                     else "⬇️ Download Excel")
            st.download_button(
                label, data=st.session_state["xlsx_bytes"],
...
# ═══════════════════════════════════════════════════════════════
    ...
    uploaded_files = st.file_uploader(
        ...
        type=["xlsx","xls"], accept_multiple_files=True, key="batch_upload",
    )
    ...
    cvs_tolerances = render_cvs_tolerance_ui(
        sorted(all_cvs_levels), unit, key_prefix="batch_")
    ...
    per_file_key_prefix: dict = {
        fname: f"batch_{_hl_a.md5(fname.encode()).hexdigest()[:8]}_"
    ...
    with st.form("batch_cvs_assign_form", clear_on_submit=False):
        for fname, prev in per_file_preview.items():
            ...
            with st.expander(f"📁 {fname}",
                              expanded=(len(per_file_preview) <= 2)):
                if not cvs_names_f:
                ...
                if not analyte_list_f:
                    ...
                    continue
                render_cvs_analyte_assignment_editor(
                    cvs_names_f, analyte_list_f,
                    key_prefix=per_file_key_prefix[fname],
                    name_to_conc=name_to_conc_f, unit=unit,
                )
    ...
    per_file_assignment: dict = {
        fname: get_applied_cvs_assignment(per_file_key_prefix[fname])
        for fname in per_file_preview
    }
        ...
        try:
            ...
            for fname, prev in per_file_preview.items():
                _std_f, _cvs_f = recompute_qc(
                    prev.to_json(), unit, cvs_tolerances,
            ...
            st.caption("Row labels prefixed with `[file_stem]` so each file's CVS / STD injections are identifiable.")
    ...
    batch_use_dil, batch_inc_blanks, batch_inc_std, batch_inc_cvs = \
        render_inclusion_checkboxes(key_prefix="batch_")
    ...
    if st.button("▶️ Run Workup", type="primary"):
        ...
        for i, f in enumerate(uploaded_files):
            ...
            try:
                ...
                wk_df, wk_types, wk_meta = build_workup(
                    res["tidy"], lloq_map, uloq_map, unit,
                    std_lo=std_lo, std_hi=std_hi,
                    cvs_tolerances=cvs_tolerances,
                    analyte_assignment=file_assignment,
                )
                ...
                for rt in wk_types:
                    ...
                    elif rt == "blank":    _kept_mask.append(batch_inc_blanks)
                    elif rt == "std":      _kept_mask.append(batch_inc_std)
                    elif rt == "cvs":      _kept_mask.append(batch_inc_cvs)
                    else:                  _kept_mask.append(False)
        ...
        status.empty()
        if not all_r["tidy"]:
            st.error("No files processed successfully."); st.stop()
            ...
            for fi, (df_, t_, m_) in enumerate(zip(per_file_dfs, per_file_types, per_file_metas)):
                if fi > 0:
                    if sep_template is None:
                        sep_template = {c: "" for c in df_.columns}
        ...
        st.session_state["batch_per_file_meta"] = {
            n: merge_run_meta(
                {**META, "Source File": n,
                 "Export Type": file_instruments.get(n, "")},
        ...
        with mc2:
            if "Pass" in comb["cvs"].columns and len(comb["cvs"]):
                ...
                st.metric("CVS Overall Pass Rate", "—")
        with mc3:
            n_blanks = int((comb["tidy"]["isStandard"] == "Blank").sum()) if "isStandard" in comb["tidy"].columns else 0
            st.metric("Blank Injections (all files)", f"{n_blanks}")
        with mc4:
            n_samples = int((comb["tidy"]["isStandard"] == "Sample").sum()) if "isStandard" in comb["tidy"].columns else 0
        ...
        for idx, fname in enumerate(per_file_names):
            with tabs[idx]:
                ...
                render_workup_table(
                    per_file_workup[idx], per_file_workup_types[idx], unit,
                ...
                render_workup_table(
                    per_file_summary[idx], per_file_summary_types[idx], unit,
    ...
    with st.expander("Advanced: SQLite export", expanded=False):
        ...
        if st.button("💾 Build SQLite DB", key="db_b"):
            with st.spinner("Writing SQLite…"):
                ...
                runs_meta_list = [
                    {**(_pf_meta_b.get(n) or {}), "Source File": n}
                    for n in _pf_names_b
                ]
                st.session_state["batch_db_bytes"] = export_sqlite(
                    runs_meta_list, _comb["tidy"]
                )
        if st.session_state["batch_db_bytes"]:
            ...
            st.download_button(
                ...
                file_name=_b_db_fn,
                mime="application/octet-stream", key="dl_db_b",
            )
            st.caption("Tables: `runs` · `tidy_data`")
